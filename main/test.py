...
while True:
    ...
    rcolor = tuple(int(x * 255) for x in hsv_to_rgb(random.random(), 1, 1)) # random color 생성
    draw.text((20, 150), "Hello World", font=fnt, fill=rcolor) # 텍스트 그리기

    rcolor = tuple(int(x * 255) for x in hsv_to_rgb(random.random(), 1, 1))
    draw.text((40, 180), "Hello World", font=fnt, fill=rcolor)

    rcolor = tuple(int(x * 255) for x in hsv_to_rgb(random.random(), 1, 1))
    draw.text((30, 210), "Hello World", font=fnt, fill=rcolor)


    # Display the Image
    disp.image(image)

    time.sleep(0.01) # 화면 갱신 속도